...
contract DeployDEX_v2 is Script {
    ...
    function deployTokens() internal {
        ...
        // Also mint the reward allocation to the deployer so `fundRewards` (which does
        // `transferFrom(msg.sender, ...)`) can successfully pull the tokens.
        // We reduced the deployer mint above so INITIAL + TOTAL_REWARDS <= Kanari.MAX_SUPPLY (11M).
        // Mint rewards to the configured FEE_RECIPIENT so the ledger/broadcast account
        // (which is often the fee recipient) holds the reward tokens and can approve
