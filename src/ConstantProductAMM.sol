...
contract ConstantProductAMM is ReentrancyGuard {
    ...
    event Swap(
        address indexed trader, address tokenIn, uint256 amountIn, address tokenOut, uint256 amountOut, uint256 fee
    );
    ...
    function addLiquidity(uint256 amountA, uint256 amountB, uint256 minAmountA, uint256 minAmountB, uint256 deadline)
        external
        payable
        nonReentrant
        returns (uint256 lpMinted)
    {
        ...
        uint256 reserveABefore =
            _isNative(tokenA) ? (address(this).balance - msg.value) : IERC20(tokenA).balanceOf(address(this));
        uint256 reserveBBefore =
            _isNative(tokenB) ? (address(this).balance - msg.value) : IERC20(tokenB).balanceOf(address(this));
    ...
    function removeLiquidity(uint256 lpAmount, uint256 minAmountA, uint256 minAmountB, uint256 deadline)
        external
        nonReentrant
    ...
    function _calculateSwapOutput(uint256 amountIn, uint256 reserveIn, uint256 reserveOut)
        internal
        view
        returns (uint256)
    {
